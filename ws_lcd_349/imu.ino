/* ===========================================================================
   ...
   The board carries a QMI8658 6-axis IMU on the main I2C bus (SDA/SCL
   47/48, the same bus as the I/O expander, codec and RTC) — a different bus
   entirely from the touch glass's own Wire1. Only the accelerometer is
   read; the gyroscope stays off — a shake gesture only needs to know how
   hard the board is moving, not which way it's turned.

   Shaking the device three times forces the same immediate poll a 2 s hold
   on the right two parts already does (net.ino's refreshNow()) — a second
   way to ask for one, not a different action. One rule keeps the two from
   being confused: a short, quiet confirmation chirp (sound.ino's
   shakeSound()) plays only when a shake is what triggered the poll — never
   for the hold, and never for the poll that happens on its own when the
   cycle wraps.
