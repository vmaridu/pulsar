...
bool netFetch(){
  ...
  if (!configHasEndpoint()){
    LOG("net", "no endpoint configured - hold the left part 2 s, join the hotspot, set the URL");
    setFault("SETUP", "hold left part 2s");
    lastPoll = millis();
    return false;
  }
  if (!net.connected){
  ...
  http.addHeader("X-Device-Board", "ws_lcd_349");
  ...
  LOG("net", ">   X-Device-Board: ws_lcd_349");
  if (configSigned()){
...
void netBegin(){
  if (!configHasEndpoint())  setFault("SETUP", "hold left part 2s");
...
/* A poll the person asked for — the glass held 2 s on the right two parts,
   or three shakes (imu.ino). Ignores the schedule, lights the status
   hairline and sweeps the graph back in. A forced refresh also clears a
   Retry-After hold: a person asking by hand outranks a backend that asked
   us to wait. `why` is just what goes in the log line — every caller
   names its own gesture.                                                 */
...
void cycleTick(){
  ...
  screenT0 = now;                             /* also resyncs the alert flash — ws_lcd_349.ino */
