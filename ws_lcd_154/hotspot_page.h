...
<section>
  <h2>Display</h2>
  <label>Brightness on battery — <span id="brightBatteryVal">50</span>%
    <input id="brightBattery" type="range" min="30" max="100" step="5" value="50"></label>
...
function load(keep){
  fetch('/api/config').then(function(r){ return r.json(); }).then(function(c){
    ...
    $('brightBattery').value = String(c.brightnessBattery != null ? c.brightnessBattery : 50);
